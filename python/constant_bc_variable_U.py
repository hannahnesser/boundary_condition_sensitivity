...
for i, pert in enumerate(perts):
    ...
    ax[0].plot(pert_BC.xp, 
               ...
               label=f'{pert} ppb')
    print((pert_BC.g @ pert_BC.c) /  pert_BC.xhat)
    print((pert_BC.bc_contrib / pert_BC.xhat))
    # Then plot the zeta term (several different versions)
    # ax_a[0].plot(pert_BC.xp, np.abs(pert_BC.bc_contrib/pert_BC.xhat),
...
for i, sf in enumerate(sfs):
    ...
    # ax_a[1].plot(pert_sa_BC.xp, np.abs(pert_sa_BC.bc_contrib/pert_sa_BC.xhat),
    #              color=fp.color(2*i, lut=10), ls='--', lw=1)
...
for i, sf in enumerate(sfs):
    ...
    #                   color=fp.color(2*i, lut=10), ls='--', lw=1)
    ax_a[-1].plot(xp, np.abs(g.sum(axis=1)/xhat),
                 color=fp.color(2*i, lut=10), ls=':', lw=1)
...
fp.add_title(ax[-1], r'Scaling $\sigma_{A,0}$ with larger buffer grid cell')
fp.add_legend(ax[-1], bbox_to_anchor=(1.15, 0.5), loc='center left')
