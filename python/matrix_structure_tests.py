...
#  XA
nstate = 20
xa = 100/3600 # ppb/day in ppb/s

# SA
sa_err_base = 50/3600 # absolute error ppb/day
sa_base = np.diag(sa_err_base*np.ones(nstate))**2

# SO
so_err_base = 15
so_base = np.diag(so_err_base*np.ones(nstate))**2
...
# print(g_base == 0)
band_width_base = band_width(k_base, so_base, sa_base)
ils_base = influence_length(k_base, so_base, sa_base)
base = [band_width_base, ils_base, ils_base]

# print(g_base*1e2*1e2*3600) # G * number of obs * perturbation error * unit conversion

x = np.arange(-1, 1.01, 0.01)
tau_effect = np.zeros((len(x), 2))
sa_effect = np.zeros((len(x), 3))
so_effect = np.zeros((len(x), 3))
for ind, i in enumerate(x):
    # Alter lifetime
    k = (10**i)*copy.deepcopy(k_base)
    tau_effect[ind, 0] = band_width(k, so_base, sa_base)
    tau_effect[ind, 1] = influence_length(k, so_base, sa_base)

    # Alter Sa
    sa = (10**(2*i))*copy.deepcopy(sa_base)
    sa_effect[ind, 0] = band_width(k_base, so_base, sa)
    sa_effect[ind, 1] = influence_length(k_base, so_base, sa)

    sa = copy.deepcopy(sa_base)
    sa[0, 0] *= (10**(2*i))
    # sa_effect[ind, 2] = band_width(k_base, so_base, sa)
    ...
    so_effect[ind, 1] = influence_length(k_base, so, sa_base)

    so = copy.deepcopy(so_base)
    so[0, 0] *= (10**(2*i))
    # so_effect[ind, 2] = band_width(k_base, so, sa_base)
...
# suffix = ['bw', 'ils']
# title = ['Sensitivity to ', ]
yaxis = ['Gain matrix\nband width', 'Influence\nlength scale',
         'Influence\nlength scale']
ylim = [(1.5, 10.5), (0.5, 5.5), (0.5, 5.5)]
fig_summ, ax_summ = fp.get_figax(aspect=2, cols=3)
...
for i in range(3):
    ...
    if i < 2:
        ax_summ[i].plot(10**x, tau_effect[:, i], ls='--', c=fp.color(2),
                     zorder=10, label='Lifetime')
    ax_summ[i].plot(10**x, sa_effect[:, i], c=fp.color(8),
                 label='Prior error')
    ax_summ[i].plot(10**x, so_effect[:, i], c=fp.color(5),
